...
Contenedor_Resultados::Contenedor_Resultados(Vector_Protegido* vector){
      this->vector_de_resultados = vector;
}
...
void Contenedor_Resultados::cargar_resultados(const std::string& nombre,
    ...
    if (nombre != "" && (resultado == HAY_CICLO || resultado == GOOD
        ...
        this->vector_de_resultados->cargar_resultados_si_son_validos(resultados);
    }
}
